...
////    public String checkoutSingleProduct(@RequestParam Long productId, Model model, HttpSession session) {
...
////
////        Map<Product, Long> cartProducts = new HashMap<>();
////        cartProducts.put(product, 1L);
////        Double totalAmount = product.getPrice();
////
////        setSessionAttributes(session, cartProducts, totalAmount);
////        setModelAttributes(model, cartProducts, totalAmount, null);
////
////        model.addAttribute("singleProductCheckout", true);
////
////        return "guest/searchpage/checkout";
////    }
////
...
////
////
////        if (productIds != null && productIds.length == 1 && quantities != null && quantities.length == 1) {
////            // Xử lý trường hợp một sản phẩm
////            Long productId = Long.parseLong(productIds[0]);
////            Product product = productServices.getProductById(productId);
////            cartProducts = new HashMap<>();
////            cartProducts.put(product, Long.parseLong(quantities[0]));
////            totalAmount = Double.parseDouble(total);
////        } else {
////            // Xử lý trường hợp nhiều sản phẩm
////            cartProducts = createCartProducts(productIds, quantities);
////            totalAmount = Double.parseDouble(total);
////        }
////
////        setSessionAttributes(session, cartProducts, totalAmount);
...
//
//    @GetMapping("/single-product")
//    public String checkoutSingleProduct(@RequestParam Long productId, Model model, HttpSession session, Principal principal) {
//        Product product = productServices.getProductById(productId);
//        if (product == null) {
//            return "redirect:/error";
//        }
...
//        Double totalAmount = product.getPrice();
//
//        // Đặt thuộc tính phiên (session) cho giỏ hàng và tổng tiền
//        setSessionAttributes(session, cartProducts, totalAmount);
//
...
//                model.addAttribute("user", user);
//
//                // Áp dụng giảm giá cho người dùng đã đăng nhập
//                Discount discount = applyDiscount(totalAmount);
//                if (discount != null) {
//                    model.addAttribute("discount", discount);
//                    model.addAttribute("discountedAmount", totalAmount - discount.getDiscountValue());
//                }
//
//                // Tính tổng số tiền mà người dùng đã mua trước đây
//                double totalSpent = calculateTotalSpent(user);
//                model.addAttribute("totalSpent", totalSpent);
//            }
//        }
...
//
//        // Đặt thuộc tính mô hình (model) cho trang hiển thị
//        setModelAttributes(model, cartProducts, totalAmount, principal);
//        model.addAttribute("singleProductCheckout", true);
//
//        return "guest/searchpage/checkout";
...
public class GuestCheckOutController {
    ...
    public String cancelOrder(@RequestParam("orderID") Long orderId, RedirectAttributes redirectAttributes, Principal principal) {
        if (principal == null) {
            return "redirect:/login";
        }
        ...
        try {
            ...
            sendCancelEmail(order, user.getEmail());
    ...
    public String createOrder(@Valid @ModelAttribute("orderDTO") OrderDTO orderDTO,
        ...
        try {
            ...
            clearCart(session);
            redirectAttributes.addFlashAttribute("orderSuccess", true);
            return "redirect:/client/order?orderSuccess=true";
    ...
    private void sendConfirmationEmail(Orders order, String email) {
        String emailContent = String.format(
                "Dear %s,\n\n" +
                        "Thank you for your order with FMOBILE. We are pleased to confirm that your order has been received and is being processed.\n\n" +
                        "Order Details:\n" +
                        "Order ID: %s\n" +
                        "Order Date: %s\n" +
                        "Shipping Address: %s\n" +
                        ...
                        "Total Amount: $%.2f\n" +
                        "Payment Method: %s\n\n" +
                        "We will notify you once your order has been shipped. If you have any questions about your order, please contact our customer support team.\n\n" +
                        "Thank you for choosing FMOBILE!\n\n" +
                        "Best regards,\n" +
                        "FMOBILE Customer Service Team",
                order.getUser() != null ? order.getUser().getFullName() : order.getFullName(),
                order.getId(),
                order.getOrderDate(),
                order.getShippingAddress(),
        ...
        mailService.sendMail(email, "Order Confirmation - FMOBILE", emailContent);
    }

    private void sendCancelEmail(Orders order, String email) {
        String emailContent = String.format(
                "Dear %s,\n\n" +
        ...
        mailService.sendMail(email, "Order Cancellation Notification", emailContent);
    }
